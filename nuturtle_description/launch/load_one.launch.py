...
from launch.substitutions import Command, PathJoinSubstitution, \
      TextSubstitution, LaunchConfiguration, EqualsSubstitution
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument(name="use_jsp", default_value="true",
                              description="true (default): use jsp to publish \
                                joint states, otherwise no joint states \
                                published"),
        ...
        DeclareLaunchArgument(name="color", default_value="purple",
                              description="purple (default): set the color of \
                                the robot",
        ...
        Node(
            ...
            arguments=["-d",
                       ...
                       "-f", [LaunchConfiguration("color"), TextSubstitution(
                           text="/"), TextSubstitution(text="base_footprint")]
                       ],
            condition=IfCondition(
                    EqualsSubstitution(LaunchConfiguration("use_rviz"),
                                       "true")),
            on_exit=Shutdown()
        )
    ])
